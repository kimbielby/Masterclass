...
class GreenSpillDataset(Dataset):
    def __init__(self, spill_images, gt_images, use_extra_channels):
    ...
    def __getitem__(self, idx):
        ...
        target_tensor = torch.tensor(gt_rgb).permute(2, 0, 1)   # (C, H, W)

        return input_tensor, target_tensor

def get_dataloader(spill_images, gt_images, batch_size, num_workers=2, use_extra_channels=False):
    dataset = GreenSpillDataset(spill_images=spill_images, gt_images=gt_images, use_extra_channels=use_extra_channels)
